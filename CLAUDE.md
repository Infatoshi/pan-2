...
- conv_gelu Triton path is DISABLED by default (`PAN2_CONV_GELU_TRITON=1` opts in): its block1 dgrad kernel emits nan dx from finite inputs at ~1% of backward calls (real-data nan at step 3-13; instrumented OUTPUT-nan at call 112 with inputs verified finite; persists with zero-init dx, so computed not stale-buffer; isolated unit tests pass, so real-loop state matters). The earlier "conv_gelu+FusedAdamW+compile triad" observations were allocator-layout artifacts of this same flake - real-data runs nan with adamw off AND compile off. `train_steps` now hard-raises on non-finite loss. Root cause open; re-enable only after a rewrite passes 5x 60-step real-data runs. Perf claims must state whether the conv path was on or off.
