...
- Compile NaN landmine (2026-07-15, root cause open): conv_gelu + FusedAdamW + inductor-compiled temporal together NaN the SECOND model+eval in a process (first NaN at `_ConvGeluBackward`; both GPU0 and GPU1; cache-independent; eager clean; single model clean). Production rules: GPU0 uses `PAN2_TEMPORAL_COMPILE_MODE=default` (clean 4/4; `reduce-overhead` NaNs there; upside only ~0.34 ms/step anyway). GPU1 runs must set `PAN2_TEMPORAL_COMPILE=0` (any compile state NaNs on sm_86). Do not retry `cudagraph_mark_step_begin` (disproven) or blame the inductor cache (fresh caches reproduce it).
